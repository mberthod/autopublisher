// Publication LinkedIn via l'API interne GraphQL (format capturé sur le vrai
// site), depuis le navigateur de l'utilisateur (session native valide).
// Publie en tant que PAGE (origin ORGANIZATION + nonMemberActorUrn) ou en tant
// que profil (origin FEED).
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  publishViaApi(msg.task)
    .then((result) => sendResponse(result))
    .catch((err) => sendResponse({ status: "failed", error_code: "UNKNOWN", error_message: err.message }));
  return true;
});

// queryId de la mutation de création de post (capturé ; peut évoluer côté LinkedIn)
const CREATE_QUERY_ID = "voyagerContentcreationDashShares.80089eb2e82a2dfa23cb621fb09eb7bf";
const CREATE_URL = `https://www.linkedin.com/voyager/api/graphql?action=execute&queryId=${CREATE_QUERY_ID}`;
...
// Récupère les en-têtes LinkedIn réels stockés par li-capture.js dans le <html>.
// needPageInstance = attendre le contexte page admin (requis pour publier en page).
...
async function publishViaApi(task) {
  const csrf = getCsrf();
  if (!csrf) {
    return { status: "failed", error_code: "AUTH_REQUIRED", error_message: "JSESSIONID introuvable — reconnecte-toi à LinkedIn" };
  }
  ...
  // Image : nécessite un mediaUrn (digitalmediaAsset) via upload interne.
  if (task.media_urls?.length) {
    try {
      ...
      console.warn("[LI] upload image échoué, publication en texte:", e.message);
    }
  }

  const body = { variables: { post }, queryId: CREATE_QUERY_ID, includeWebMetadata: true };

  // En-têtes réels de LinkedIn (bonne version + contexte page admin). Pour une page,
  // on attend le x-li-page-instance de l'espace admin.
  const li = await waitForLiHeaders(12000, !!orgId);
  ...
  if (r.status === 401 || r.status === 403) {
    return { status: "failed", error_code: "AUTH_REQUIRED", error_message: `HTTP ${r.status} (session LinkedIn expirée ?)` };
  }
  const raw = await r.text().catch(() => "");
  if (!r.ok) {
    return { status: "failed", error_code: "PUBLISH_REJECTED", error_message: `HTTP ${r.status}: ${raw.slice(0, 250)}` };
  }
  // 200 mais échec applicatif : LinkedIn renvoie une erreur dans le corps
  if (/createContentcreationDashShares":null|"exceptionClass"|FORBIDDEN|serviceErrorCode/i.test(raw)) {
    return { status: "failed", error_code: "PUBLISH_REJECTED", error_message: `Réponse LinkedIn: ${raw.slice(0, 1500)}` };
  }
...
// Upload d'image (best-effort) — le format d'upload sera affiné après capture.
