// Publication LinkedIn via l'API interne "Voyager", depuis le navigateur de
// l'utilisateur (session native, valide). On n'automatise plus le DOM : on
// appelle directement l'endpoint que le site LinkedIn utilise en interne.
// Le content script s'exécute dans le contexte de linkedin.com → le fetch part
// avec la bonne origine, les cookies et le jeton csrf.
...
async function publishViaApi(task) {
  ...
  // Upload d'un éventuel média (image) puis référence dans le post
  let media = [];
  if (task.media_urls?.length) {
    try {
      ...
      // On publie quand même en texte si l'upload échoue, mais on le signale
      console.warn("[LI] upload image échoué:", e.message);
    }
  }
...
// Upload d'image via le flux interne : registerUpload → PUT bytes → mediaUrn
